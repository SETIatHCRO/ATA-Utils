...
def printHelp()
  puts "Syntax:";
  puts "  ./bf_testing.rb <delay cal target> <phase target name> <BF1 freq MHz> <BF2/3 freq MHz> <beam1 target name> <beam2 target name> <beam3 target name> <logfile name tag>";
  puts " Example:";
  puts "  ./bf_testing.rb casa 3c48 1680.0 1680.0 moon moon moon testlog1"; 
end
...
def allFreqCal(centerFreq, offset)
  ...
  t1 = Thread.new{freqCal(1, centerFreq.to_f + offset)};
  t2 = Thread.new{freqCal(2, centerFreq.to_f + offset)};
  #t3 = Thread.new{freqCal(3, centerFreq.to_f + offset)};
  t1.join;
  t2.join;
  #t3.join;
...
puts " Delay target:   " + $delayTarget + " @ " + $delayFreq.to_s;
puts " Phase target:   " + $phaseTarget;
...
# BFreset
t1 = Thread.new{bfReset(1)};
t2 = Thread.new{bfReset(2)};
#t3 = Thread.new{bfReset(3)};
t1.join();
t2.join();
#t3.join();
...
cmd = "atasetazel \"" +  $antList + " 330.0 30.0 -w \"";
doCmd(cmd);
t1 = Thread.new{bfAttn(1)};
t2 = Thread.new{bfAttn(2)};
#t3 = Thread.new{bfAttn(3)};
t1.join();
t2.join();
#t3.join();
...
#t1 = Thread.new{bfInit(1, "10.1.50.52", 50000, "10.1.50.116", 50001, $antPolList[0])};
#t2 = Thread.new{bfInit(2, "10.1.50.112", 50000, "10.1.50.112", 50001, $antPolList[1])};
#t3 = Thread.new{bfInit(3, "10.1.50.108", 50000, "10.1.50.108", 50001, $antPolList[2])};
t1 = Thread.new{bfInit(1, "10.1.50.100", 50000, "10.1.50.100", 50001, $antPolList[0])};
t2 = Thread.new{bfInit(2, "10.1.50.104", 50000, "10.1.50.104", 50001, $antPolList[1])};
#t3 = Thread.new{bfInit(3, "10.1.50.108", 50000, "10.1.50.101", 50000, $antPolList[2])};
t1.join();
t2.join();
#t3.join();
...
$reportText ="";
createEphem($delayTarget, 1, false);
createEphem($delayTarget, 2, false);
createEphem($delayTarget, 3, false);
track($delayTarget);

for i in 0..1
if(i == 1) then $reportText =""; end
t1 = Thread.new{delayCal(1, false)};
t2 = Thread.new{delayCal(2, false)};
#t3 = Thread.new{delayCal(3, false)};
t1.join;
t2.join;
#t3.join;
end
...
createEphem($phaseTarget, 1, false);
createEphem($phaseTarget, 2, false);
createEphem($phaseTarget, 3, false);
track($phaseTarget);

$reportText = "";
t1 = Thread.new{phaseCal(1, $obsFreq1)};
t2 = Thread.new{phaseCal(2, $obsFreq2)};
#t3 = Thread.new{phaseCal(3, $obsFreq2)};
t1.join;
t2.join;
#t3.join;
...
createEphem($obsTarget[0], 1, true);
createEphem($obsTarget[1], 2, true);
createEphem($obsTarget[2], 3, true);
track($obsTarget[0]);
...
t1 = Thread.new{pointBF(1, $obsFreq1)};
t2 = Thread.new{pointBF(2, $obsFreq2)};
#t3 = Thread.new{pointBF(3, $obsFreq2)};
t1.join;
t2.join;
#t3.join;
