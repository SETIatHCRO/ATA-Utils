...
def main():
    ...
    ofile = open("/home/obsuser/pointing_new/%i_atapointer.txt" %t, "w")
    tpoints = {ant:open("/home/obsuser/pointing_new/%i_%s_obs.txt" %(t,ant), "w") 
            for ant in ant_list}
        ...
        if to_observe:
            ...
            for i in range(nRepeats):
                ...
                for ant in ant_list:
                    ...
                    meas_az = (az_avg_src + peak_pos_az)
                    meas_el = (el_avg_src + peak_pos_el)

                    #meas_az, meas_el, ir = pms[ant].applyTPOINTCorrections(meas_az, meas_el, 0)
                    dt = datetime.datetime.now()
                    hour = dt.hour + dt.minute/60.

                    tpoints[ant].write(("%.4f "*10+"\n")
                            %(az_avg, el_avg, az_avg_src, el_avg_src, peak_pos_az, peak_pos_el, peak_width_az, peak_width_el, peak_sbr, hour))
                    tpoints[ant].flush()
