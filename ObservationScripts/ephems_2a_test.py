...
def generate_ephem_el_swivel(az_start, el_start, el_end, t_start, t_span,  steps, invr):
    ...
    -------
    ephem : numpy_array
            Returns an array with 4 columns for time in ns, azimuth, elevation 
...
def generate_ephem_az_swivel(az_start, az_end, el_start, t_start, t_span,  steps, invr):
    ...
    This function creates an ephemeris array to enable the antennas to swivel across elevation at a particular azimuth

     Parameters: az_start : float
                            azimuth start
                 az_end   : float
                            azimuth end
                 el_start : float
     ...
     Returns: ephem : numpy_array
                      Returns an array with 4 columns for time in ns, azimuth, elevation and inverse radius of the source
    """
    tai = np.linspace(t_start*1e9, (t_start + t_span)*1e9, steps)
...
def ephem_to_txt(save_as, ephem_file):
    ...
    This function exports the ephemeris array to a txt file
     ...
     Parameters: save_as    : string
                              file name to save the array as
                 ephem_file : array
                              the ephemeris array to be exoprted as a txt file
