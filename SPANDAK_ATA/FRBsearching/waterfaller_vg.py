...
def plot_waterfall(data, start, source_name, duration, dm,ofile,
    ...
    zapthresh=4
    if extrazap:
        off_spec1 = np.array(data.data[..., 0:int(burst_bin-window_width)]) # off-pulse from left
        off_spec2 = np.array(data.data[...,int(burst_bin+window_width):nbinlim]) # off-pulse from right
    ...
    #print("!!!")
    #print(burst_bin)
    #print(times[int(burst_bin)])
    #print(spectrum_window)
    ax_ts.axvline(times[int(burst_bin)]-spectrum_window,ls="--",c="grey")
    ax_ts.axvline(times[int(burst_bin)]+spectrum_window,ls="--",c="grey")

    #Get spectrum and DM-vs-SNR for the on-pulse window	
    on_spec = np.array(data.data[..., int(burst_bin-window_width):int(burst_bin+window_width)])
    on_dmsnr = np.array(dmvstm_array[..., int(burst_bin-window_width):int(burst_bin+window_width)])
    ...
    off_spec2 = np.array(data.data[...,int(burst_bin+window_width):nbinlim])	# off-pulse from right 
    off_spec  = (np.mean(off_spec1,axis=1) + np.mean(off_spec2,axis=1))/2.0 # Total off-pulse 	
    off_dmsnr1 = np.array(dmvstm_array[...,0:int(burst_bin-window_width)])
    off_dmsnr2 = np.array(dmvstm_array[...,int(burst_bin+window_width):nbinlim])
