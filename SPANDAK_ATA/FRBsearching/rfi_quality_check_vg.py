...
def rfi_check(base_name, mask_file, rfitime, nchans, totime, chanfrac,intfrac):
	...
	#jamar's solution
	#for k,g in groupby(enumerate(kill_chans), lambda x: x[1]-x[0]):

	#sofia's solution, based on a Python 2->3 converter - looks like indices are switched vs. Jamar's solution
